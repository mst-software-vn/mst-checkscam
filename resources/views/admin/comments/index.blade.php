  ...
  <div class="card">
    <div class="card-body">
      ...
      <div
        ...
        id="filter_inputs"
        style="
          display: {{ request()->hasAny(['report_id', 'keyword', 'is_anonymous', 'time_range']) ? 'block' : 'none' }};
          ...
          <form method="GET" action="{{ route('admin.comments.index') }}">
            <div class="row g-3 align-items-end">
              <div class="col-lg-2 col-md-6">
                <label class="form-label fw-bold small text-muted mb-2">ID Báo cáo</label>
                <input
                  type="text"
                  name="report_id"
                  class="form-control"
              ...
              <div class="col-lg-3 col-md-6">
                <label class="form-label fw-bold small text-muted mb-2">Tìm kiếm nội dung</label>
              ...
              <div class="col-lg-2 col-md-6">
                ...
                <select name="is_anonymous" class="form-select select2-basic">
                  <option value="">Tất cả</option>
                  <option value="1" {{ request('is_anonymous') === '1' ? 'selected' : '' }}>Ẩn danh</option>
                  <option value="0" {{ request('is_anonymous') === '0' ? 'selected' : '' }}>Có tên</option>
                </select>
              </div>
              <div class="col-lg-3 col-md-6">
          ...
          <tbody>
            @forelse ($comments as $comment)
              <tr>
                <td>
                  <label class="checkboxs">
                    <input type="checkbox" class="check-item" value="{{ $comment->id }}" />
                    <span class="checkmarks"></span>
                  </label>
                </td>
                <td>
                  ...
                  @else
                    <span class="text-muted">#RPT{{ str_pad($comment->report_id, 3, '0', STR_PAD_LEFT) }}</span>
                  @endif
                </td>
                <td>
    ...
    <div class="modal-dialog modal-dialog-centered">
      <div class="modal-content" style="border-radius: 16px; border: none; overflow: hidden">
        <div class="modal-header border-0 pb-0" style="background: linear-gradient(135deg, #fff5f5 0%, #fff 100%)">
          <div class="d-flex align-items-center w-100 gap-3 px-1 pt-1">
            ...
            <div>
              <h5 class="modal-title fw-bold mb-0">
                Xóa bình luận #
                <span id="single-comment-id">0</span>
              </h5>
        ...
        <div class="modal-body px-4 py-4">
          ...
          <p class="text-muted mb-0" style="font-size: 14px">
            Bạn có chắc chắn muốn xóa bình luận này? Nội dung bình luận sẽ bị
  ...
  @push('scripts')
    <script>
      document.addEventListener('DOMContentLoaded', function () {
        ...
        const singleCommentIdSpan = document.getElementById('single-comment-id');
        ...
        document.addEventListener('click', function (e) {
          ...
          if (btn) {
            ...
            singleCommentIdSpan.textContent = currentDeleteId;
            singleModal.show();
          }
        });
        ...
        if (confirmSingleDeleteBtn) {
          confirmSingleDeleteBtn.addEventListener('click', function () {
            ...
            setTimeout(() => {
              fetch(`{{ url('admin/comments') }}/${currentDeleteId}`, {
                method: 'DELETE',
                headers: {
                  'X-CSRF-TOKEN': '{{ csrf_token() }}',
